...
loglike = function(para_un, m.spot) {
    ...
    theta  = as.matrix(c(para[16:18], home_value_houses_nsw$log_index[1], asx200$log_price[1]))
    lambda = para[19]
...
# init_para_un = c(0.3696448117, -0.0226900395,  0.7603241807, -0.1382333940, -0.1212550658, -0.0120939080,  0.2048884876 , 0.2570906817, 0.5953711481, -0.5581090375,  0.5621663040, -0.0773762896,  1.1323443970, -0.4059042990, -0.1079379843,  0.3634425387, 0.0045291023,  0.6044371074, -0.0515580273,  0.0122061338,  0.0114722658,  0.0096651618,  0.0085276468,  0.0068691666, 0.0059631228,  0.0052143616,  0.0047095021,  0.0041040807, -0.0041710564,  0.0038269661,  0.0033805255,  0.0033465011,  0.0027621710,  0.0027256280,  0.0025578855,  0.0022913802,  0.0019411415,  0.0016112021, -0.0015108390, -0.0013903791,  0.0010614574, -0.0008603987,  0.0006777453,  0.0005330642,  0.0004203710, -0.0002549369,  0.0001683272, -0.0000954686,  0.0000584604,  0.0000953540,  0.0001625798,  0.0002451214,  0.0003346435,  0.0004616734,  0.0005425149,  0.0006963937,  0.0008938731,  0.0011882339,  0.0013112355, -0.0039078314, -0.0212962482)

load("param_gindep.RData")
load("Xt_gindep_fitted.RData")
...
stopCluster(cl)
# save(param_gindep, file = "param_gindep.RData")
# save(Xt_gindep_fitted, file = "Xt_gindep_fitted.RData")
...
theta_est  = as.matrix(c(param_gindep$par[16:18], home_value_houses_nsw$log_index[1], asx200$log_price[1]))
lambda_est = param_gindep$par[19]
...
zcp_gindep_sim = replicate(n = num_paths,
                         expr = {matrix(NA, nrow = num_year/h+1, ncol = 40)},
                         simplify = F)
for (path in 2:num_paths) {
  zcp_gindep_sim[[path]] = t(B_gindep_fitted %*% as.matrix(Xt_gindep_sim[[path]])) - A_gindep_fitted
}
zcp_gindep_sim = lapply(zcp_gindep_sim, function(x){ x = as.data.frame(x); x[1,] = zcp_month[nrow(zcp_month), -1]; return (x)})
...
trans = function(b) {
    # parameter restrictions of independent factor
    bb = b
    # bb[10:16] = 1/(1+exp(b[10:16])) # rhos
    bb[c(10,12,15,19,24)] = b[c(10,12,15,19,24)] ^ 2 # sigmas
    bb[28] = b[28]^2          # lambda
    bb[29:npara] = b[29:npara]^2          # measurement error
    return(bb)
}
...
loglike = function(para_un, m.spot) {
    ...
    theta  = as.matrix(c(para[25:27], home_value_houses_nsw$log_index[1], asx200$log_price[1]))
    lambda = para[28]
    H      = diag(para[29:npara])
    ...
    Q_integrand = function(s) { expm(-s * kappa) %*% sigma %*% t(sigma) %*% expm(-s * t(kappa)) }
    xt_term_integrand = function(s) {expm(-s * kappa)}
    integrate_matrix = function (from = 0, to, num_step = 10) {
      results = 0
      step = (to-from)/num_step
      res_step = 0
      for (i in seq(from = from+step, to = to, by = step) ) {
    ...
    integrate_matrix2 = function (from = 0, to, num_step = 10) {
      results = 0
      step = (to-from)/num_step
      res_step = step
      for (i in seq(from = from+step, to = to, by = step) ) {
    ...
    Q = integrate_matrix(from = 0, to = dt, num_step = 10)
    Q0 = integrate_matrix(from = 0, to = 10, num_step = 120)
    xt_term = integrate_matrix2(from = 0, to = dt, num_step = 10)
...
# init_para_un = c(rep(c(log(1/param_indep$par[1]-1), param_indep$par[2], param_indep$par[3]), 3), # kappa
#                  -param_indep$par[4:6], # rho4
#                  param_indep$par[4:6], param_indep[4], # rho5
#                  5.77261e-06,  1.00538e-05, # sigma4, sigma 5
#                  rep(param_indep$par[4:6],2), # sigma
#                  param_indep$par[7:9], # theta
#                  sqrt(param_indep$par[10]),# lambda
#                  sqrt(rep(9e-08,nmat)))
init_para_un = c(-0.0544, -0.0167, 0.0089,
                 0.2772, 0.2903, -0.5783,
                 -0.4189, 0.0842, 0.8158, # kappa
                 0.0046,
                 -0.0057, 0.0065,
                 -0.0052, -0.005, 0.0272, 
                 -0.008,0.0038,0.0024,0.0245,
                 -0.008,0.0038,0.0024,0.0245,0.0024, # sigma
                 0.0577, -0.0167, -0.0127, # theta
                 0.4434,# lambda
                 sqrt(rep(0.36,nmat)))

# load("Xt_gcorr_fitted_new.RData")
...
save(param_gcorr, file = "param_gcorr_new.RData")
save(Xt_gcorr_fitted, file = "Xt_gcorr_fitted_new.RData")
...
(kappa_est = rbind(c(param_gcorr$par[1],param_gcorr$par[2],param_gcorr$par[3],0,0),
                  ...
                  c(-1,-1,0,0,0),
                  c(-1,-1,0,0,0)))
(Sigma_est = rbind(c(param_gcorr$par[10],param_gcorr$par[11],param_gcorr$par[12],0,0),
...
# rho41 = param_gcorr$par[10]; rho42 = param_gcorr$par[11]; rho43 = param_gcorr$par[12]
# rho44 = sqrt(1 - rho41^2 - rho42^2 - rho43^2)
# rho51 = param_gcorr$par[13]; rho52 = param_gcorr$par[14]; rho53 = param_gcorr$par[15]; rho54 = param_gcorr$par[16]
# rho55 = sqrt(1 - rho51^2 - rho52^2 - rho53^2 - rho54 ^ 2)
# sigma4 = param_gcorr$par[17]; sigma5 = param_gcorr$par[18]
# Sigma_est  = rbind(c(param_gcorr$par[19],0,0,0,0),
#                 c(param_gcorr$par[20],param_gcorr$par[21],0,0,0),
#                 c(param_gcorr$par[22],param_gcorr$par[23],param_gcorr$par[24],0,0),
#                 c(sigma4 * rho41, sigma4 * rho42, sigma4 * rho43, sigma4 * rho44,0),
#                 c(sigma5 * rho51, sigma5 * rho52, sigma5 * rho53, sigma5 * rho54, sigma5 * rho55))
theta_est  = as.matrix(c(param_gcorr$par[25:27], home_value_houses_nsw$log_index[1], asx200$log_price[1]))
lambda_est = param_gcorr$par[28]

# fitted Xt 
matplot(Xt_gcorr_fitted, type = "l", ylab = "L,S,C,House,Stock", lty = 1, main = "AFNS 5 Factor Estimates (L,S,C,House,Stock)", lwd = 2)
legend("topright", legend = c("level", "slope", "curvature","House","Stock"), col = 1:5, lty = 1)

# compare fitted yields 
B_gcorr_fitted = NS.B(lambda_est, v.mat)
A_gcorr_fitted = AFNS.C(Sigma_est, lambda_est, v.mat)
zcp_gindep_fitted = t(B_gindep_fitted[1:40,1:3] %*% t(Xt_gindep_fitted[,1:3])) - A_gindep_fitted
...
for (path in 1:num_paths) { # discretisation
  for (i in 2:(num_year/h+1)) {
    ...
    Xt_gcorr_sim[[path]][,i] = Xt + h * (theta_est - kappa_est %*% Xt) * h + sqrt(h) * Sigma_est %*% e
  }
}
...
for (i in 2:(num_year/h+1)) {
  ...
  Xt_gcorr_mean[,i] =Xt + h * (theta_est - kappa_est %*% Xt) * h
}
...
# house 
plot_data = c(m.spot[,41], zcp_sim$house_index[,1])
plot(plot_date, plot_data, "l", main = "Simulated house indexes", xlab = "", ylab = "", col = "grey", ylim = c(-0.04,0.1))
for (i in 2:num_paths) {
  plot_data = c(m.spot[,41], zcp_sim$house_index[,i])
  lines(plot_date, plot_data, col = "grey")
}

# stock 
plot_data = c(m.spot[.42], zcp_sim$stock_price[,1])
plot(plot_date, plot_data, "l", main = "Simulated stock prices", xlab = "", ylab = "", col = "grey", ylim = c(-0.04,0.1))
for (i in 2:num_paths) {
  plot_data = c(m.spot[,42], zcp_sim$stock_price[,i])
  lines(plot_date, plot_data, col = "grey")
}
