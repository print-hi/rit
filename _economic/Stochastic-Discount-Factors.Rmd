...
stopCluster(cl)
lambdas
lambda_0 = matrix(lambdas$par[1:8], nrow = 8)
lambda_1 = matrix(lambdas$par[-c(1:8)], nrow = 8, ncol = 8, byrow = T)
# 
# save(lambdas, file = "lambdas.RData")
# load("lambdas.RData")
# lambda_0 = as.matrix(read.csv("lambda_0.csv")[,-1])
# lambda_1 = as.matrix(read.csv("lambda_1.csv")[,-1])
```
...
# }
# 
# zt_fitted = read.csv("all_data_fitted.csv")[,-1]
...
registerDoParallel(cl)
set.seed(1)
sdf = get_sdf(num_paths = num_paths, num_years = num_years)
stopCluster(cl)

par(mfrow = c(1,2))
plot(time_names_past, st_past * 100, "l", main = "Historical SDFs", xlab = "", ylab = "")
plot(as.Date(row.names(sdf)), sdf[,1] * 100, "l", main = "A sample path of simulated SDFs", xlab = "", ylab = "")
```

# Testing 

```{r}
plot_data = c(st_past[-length(st_past)], sdf[,1])*100
time_names = seq(from = as.Date("1993-04-01"), to = as.Date("2041-01-01"), by = "quarter")
plot(time_names, plot_data, "l")
# for (i in 2:10) {
#   plot_data = c(st_past[-length(st_past)], sdf[,i]) * 100
#   lines(time_names, plot_data, col = "grey")
# }
```
