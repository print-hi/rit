...
for (n in 1:1000){ # 1000 simulations
  ...
  #list of 
  state_status=list()
  state_status[[1]]=cbind(c(1,0,0,0,0)) # initial state status is 1 in the healthy state and 0 for the others
  for (age in 65:110){
...
for (age in 65:110){
  ...
  state_status[[age-63]][j,1]=state_status[[age-64]][1,1]*trans_prob_matrix[[age-64]][1,j]+ 
                              state_status[[age-64]][2,1]*trans_prob_matrix[[age-64]][2,j]+
