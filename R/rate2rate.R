...
#'
#' @param rates a matrix containing the rates
#' @param from specifies the type of rates we are converting from. Takes the
#'   following values: "central" for central death rates, "prob" for 1-year
#'   death probabilities, "force" for force of mortality.
#' @param to specifies the type of rates we are converting to. Takes the same
#'   values as the parameter \code{from}
#' @return a matrix of the same size with the converted rates
#' @export
#'
#' @examples
...
rate2rate <- function(rates, from, to) {
  if (from == "central" & to == "force") return(rates)
  else if (from == "central" & to == "prob") m2q(rates)
  else if (from == "force" & to == "central") return(rates)
  else if (from == "force" & to == "prob") mu2q(rates)
  else if (from == "prob" & to == "central") q2m(rates)
  else if (from == "prob" & to == "force") m2q(rates)
